...
void Expand(int noTer, char *separator, TGram G, 
  ...
  ValAl = (double)rand()/RAND_MAX;
  WAct = 0.0;

  reg = G.a[noTer];
  while ((reg != NULL) && (WAct != 1.0)) {
    WAct = WAct + reg->pro;
    if (WAct >= ValAl) {
      if (strcmp(separator,"   ") != 0)
	fprintf(stdout,"%c",separator[0]);
      Expand(reg->con1, separator, G, VNT, VT);
      if (strcmp(separator,"   ") != 0)
	fprintf(stdout,"%c",separator[1]);
      Expand(reg->con2, separator, G, VNT, VT);
      if (strcmp(separator,"   ") != 0)
  ...
  if (WAct != 1.0) {
    Warning(5,VNT[noTer]);
    fprintf(stderr,"%e\n",WAct);
    Expand(noTer, separator, G, VNT, VT);
  }
}
