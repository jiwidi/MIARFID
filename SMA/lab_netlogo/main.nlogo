...
to initialize-professor
  create-professors num_professors [
...
to initialize-fire
ask n-of initial-fire-spots patches with [pcolor = black or pcolor = white]
  [
      sprout-fire-spots 1
      [
      set shape "fire"
...
to move-professors
  ask professors [
   ifelse count fire-spots > 0 and extinguisher-cap? > 0
    ...
    [
      if safe-professor? = false [
        set busy-professor? false
        let closest-exit (patches with [pcolor = pink])
        face min-one-of closest-exit [distance myself]
        forward 0.1
        if pcolor = pink [
          let cyan-patches (patches in-radius 4 with [pcolor = cyan] )
          move-to one-of cyan-patches
          set safe-professor? true
        ]
      ]
    ]
  ]
