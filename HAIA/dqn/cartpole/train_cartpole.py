...
def select_action(state):
    ...
    eps_threshold = final_epsilon + (initial_epsilon - final_epsilon) * math.exp(
...
def optimize_model():
    ...
    for idx, next_state in enumerate(next_state_batch):
        if done_batch[idx] == True:
            ...
            next_state_q_vals[idx] = (
                target_net(next_state_batch[idx]).max(0)[0]
            ).detach()
    ...
    for t in range(1000):
        ...
        if done:
            ...
            if solved_times == 100:
                ...
                break
            if i_episode % 100 == 0:
                eps = final_epsilon + (initial_epsilon - final_epsilon) * math.exp(
                    -1.0 * steps_done / epsilon_decay
                )
                print(eps)
