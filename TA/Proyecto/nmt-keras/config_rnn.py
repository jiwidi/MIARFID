...
def load_parameters():
    ...
    TEXT_FILES = {"train": "full_tr.", "val": "dev.", "test": "test."}  # Data files.
    ...
    INPUTS_IDS_DATASET = [
        "source_text",
        "state_below",
    ]  # Corresponding inputs of the dataset.
    ...
    INPUTS_TYPES_DATASET = [
        "text-features",
        "text-features",
    ]  # Corresponding types of the data. 'text' or 'text-features' allowed.
    OUTPUTS_TYPES_DATASET = [
        "text-features"
    ]  # They are equivalent, only differ on how the data is loaded.

    # Evaluation params
    METRICS = ["coco"]  # Metric used for evaluating the model.
    EVAL_ON_SETS = [
        "val"
    ]  # Possible values: 'train', 'val' and 'test' (external evaluator).
    EVAL_ON_SETS_KERAS = (
        []
    )  # Possible values: 'train', 'val' and 'test' (Keras' evaluator). Untested..
    START_EVAL_ON_EPOCH = 120  # First epoch to start the model evaluation.
    ...
    MAXLEN_GIVEN_X = (
        True  # Generate translations of similar length to the source sentences.
    )
    MAXLEN_GIVEN_X_FACTOR = (
        2  # The hypotheses will have (as maximum) the number of words of the
    )
    # source sentence * LENGTH_Y_GIVEN_X_FACTOR.
    MINLEN_GIVEN_X = (
        True  # Generate translations of similar length to the source sentences.
    )
    MINLEN_GIVEN_X_FACTOR = (
        3  # The hypotheses will have (as minimum) the number of words of the
    )
    # source sentence / LENGTH_Y_GIVEN_X_FACTOR.
    ...
    ALPHA_FACTOR = 0.6  # Normalization according to |h|**ALPHA_FACTOR.
    ...
    START_SAMPLING_ON_EPOCH = 100  # First epoch where to start the sampling counter.
    ...
    BPE_CODES_PATH = (
        DATA_ROOT_PATH + "/training_codes.joint"
    )  # If TOKENIZATION_METHOD = 'tokenize_bpe',
    # sets the path to the learned BPE codes.
    DETOKENIZATION_METHOD = (
        "detokenize_none"  # Select which de-tokenization method we'll apply.
    )
    ...
    MAX_OUTPUT_TEXT_LEN_TEST = (
        MAX_OUTPUT_TEXT_LEN * 3
    )  # Maximum length of the output sequence during test time.

    # Optimizer parameters (see model.compile() function).
    LOSS = "categorical_crossentropy"
    CLASSIFIER_ACTIVATION = "softmax"
    SAMPLE_WEIGHTS = (
        True  # Select whether we use a weights matrix (mask) for the data outputs
    )
    LABEL_SMOOTHING = 0.0  # Epsilon value for label smoothing. Only valid for 'categorical_crossentropy' loss. See arxiv.org/abs/1512.00567.

    OPTIMIZER = "Adam"  # Optimizer. Supported optimizers: SGD, RMSprop, Adagrad, Adadelta, Adam, Adamax, Nadam.
    LR = 0.01  # Learning rate. Recommended values - Adam 0.0002 - Adadelta 1.0.
    CLIP_C = 1.0  # During training, clip L2 norm of gradients to this value (0. means deactivated).
    CLIP_V = 0.0  # During training, clip absolute value of gradients to this value (0. means deactivated).
    USE_TF_OPTIMIZER = (
        True  # Use native Tensorflow's optimizer (only for the Tensorflow backend).
    )

    # Advanced parameters for optimizers. Default values are usually effective.
    MOMENTUM = 0.0  # Momentum value (for SGD optimizer).
    ...
    LR_REDUCER_TYPE = (
        "exponential"  # Function to reduce. 'linear' and 'exponential' implemented.
    )
    ...
    # Training parameters
    MAX_EPOCH = 16  # Stop when computed this number of epochs.
    ...
    HOMOGENEOUS_BATCHES = (
        False  # Use batches with homogeneous output lengths (Dangerous!!).
    )
    JOINT_BATCHES = (
        4  # When using homogeneous batches, get this number of batches to sort.
    )
    ...
    MIN_DELTA = 0.0  # Minimum change in the monitored quantity to consider it as an improvement.

    # Model parameters
    MODEL_TYPE = (
        "AttentionRNNEncoderDecoder"  # Model to train. See model_zoo.py for more info.
    )
    ...
    # # # # # # # # # # # # # # # # # # # # # # # #
    TRAINABLE_ENCODER = (
        True  # Whether the encoder's weights should be modified during training.
    )
    TRAINABLE_DECODER = (
        True  # Whether the decoder's weights should be modified during training.
    )
    ...
    INIT_ATT = (
        "glorot_uniform"  # Initialization function for attention mechism matrices
    )

    SOURCE_TEXT_EMBEDDING_SIZE = 2048  # Source language word embedding size.
    ...
    SRC_PRETRAINED_VECTORS_TRAINABLE = (
        True  # Finetune or not the target word embedding vectors.
    )

    TARGET_TEXT_EMBEDDING_SIZE = 2048  # Source language word embedding size.
    ...
    TRG_PRETRAINED_VECTORS_TRAINABLE = (
        True  # Finetune or not the target word embedding vectors.
    )

    SCALE_SOURCE_WORD_EMBEDDINGS = (
        False  # Scale source word embeddings by Sqrt(SOURCE_TEXT_EMBEDDING_SIZE).
    )
    SCALE_TARGET_WORD_EMBEDDINGS = (
        False  # Scale target word embeddings by Sqrt(TARGET_TEXT_EMBEDDING_SIZE).
    )
    ...
    BIDIRECTIONAL_DEEP_ENCODER = (
        True  # Use bidirectional encoder in all encoding layers.
    )
    ...
    ADDITIONAL_OUTPUT_MERGE_MODE = (
        "Add"  # Merge mode for the skip-connections (see keras.layers.merge.py).
    )
    ...
    MULTIHEAD_ATTENTION_ACTIVATION = (
        "linear"  # Activation the input projections in the Multi-Head Attention blocks.
    )
    FF_SIZE = (
        MODEL_SIZE * 4
    )  # Size of the feed-forward layers of the Transformer model.
    N_HEADS = 8  # Number of parallel attention layers of the Transformer model.
    # # # # # # # # # # # # # # # # # # # # # # # #

    # Regularizers
    REGULARIZATION_FN = (
        "L2"  # Regularization function. 'L1', 'L2' and 'L1_L2' supported.
    )
    WEIGHT_DECAY = 1e-4  # Regularization coefficient.
    RECURRENT_WEIGHT_DECAY = 0.0  # Regularization coefficient in recurrent layers.

    DROPOUT_P = 0.0  # Percentage of units to drop (0 means no dropout).
    RECURRENT_INPUT_DROPOUT_P = (
        0.0  # Percentage of units to drop in input cells of recurrent layers.
    )
    RECURRENT_DROPOUT_P = 0.0  # Percentage of units to drop in recurrent layers.
    ATTENTION_DROPOUT_P = (
        0.0  # Percentage of units to drop in attention layers (0 means no dropout).
    )

    USE_NOISE = True  # Use gaussian noise during training.
    NOISE_AMOUNT = 0.001  # Amount of noise.
    ...
    DOUBLE_STOCHASTIC_ATTENTION_REG = (
        0.0  # Doubly stochastic attention (Eq. 14 from arXiv:1502.03044).
    )
    ...
    if MODEL_TYPE == "AttentionRNNEncoderDecoder":
        MODEL_NAME = (
            ...
            + MODEL_TYPE
            + "_src_emb_"
            + str(SOURCE_TEXT_EMBEDDING_SIZE)
            + "_bidir_"
            + str(BIDIRECTIONAL_ENCODER)
            + "_enc_"
            + ENCODER_RNN_TYPE
            + "_"
            + str(ENCODER_HIDDEN_SIZE)
            + "_dec_"
            + DECODER_RNN_TYPE
            + "_"
            + str(DECODER_HIDDEN_SIZE)
            + "_deepout_"
            + "_".join([layer[0] for layer in DEEP_OUTPUT_LAYERS])
            + "_trg_emb_"
            + str(TARGET_TEXT_EMBEDDING_SIZE)
            + "_"
            + OPTIMIZER
            + "_"
            + str(LR)
        ...
        MODEL_NAME = (
            ...
            + str(N_HEADS)
            + "_encoder_blocks_"
            + str(N_LAYERS_ENCODER)
            + "_decoder_blocks_"
            + str(N_LAYERS_DECODER)
            + "_deepout_"
            + "_".join([layer[0] for layer in DEEP_OUTPUT_LAYERS])
            + "_"
            + OPTIMIZER
    ...
    EMBEDDINGS_FREQ = (
        1  # Frequency (in epochs) at which selected embedding layers will be saved.
    )
    ...
    PLOT_EVALUATION = (
        False  # If True, the evaluation will be plotted into the model folder.
    )
    ...
    TRAIN_ON_TRAINVAL = (
        False  # train the model on both training and validation sets combined.
    )
    FORCE_RELOAD_VOCABULARY = (
        False  # force building a new vocabulary from the training samples
    )
