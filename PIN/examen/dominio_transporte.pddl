(define (domain transporte)
    ...
    (:predicates ; Para definir en que ciudad esta cada cosa
        ...
        ;Furgoneta llena - Se llena con un paquete nada mas
        (fullloaded ?f - furgoneta)
    )
    ...
    (:action load-avion
        :parameters (?v - avion ?l - loc ?p - paquete)
        :precondition (and
            (at ?v ?l)
            (at ?p ?l)
        )
    ...
    (:action load-tren
        :parameters (?v - tren ?l - loc ?p - paquete)
        :precondition (and
            (at ?v ?l)
            (at ?p ?l)
        )
    ...
    (:action load-furgoneta
        :parameters (?v - furgoneta ?l - loc ?p - paquete)
        :precondition (and
            (at ?v ?l)
            (at ?p ?l)
            (not (fullloaded ?v))
        ...
        :effect (and
            (not (at ?p ?l)) (loaded ?v ?p) (fullloaded ?v)
        )
    )

    ; Descargar un paquete de un vehiculo
    (:action unload-furgoneta
        :parameters (?v - furgoneta ?l - loc ?p - paquete)
        :precondition (and
            (at ?v ?l)
            (loaded ?v ?p)
            ; (fullloaded ?v) No hace falta ya que esta loaded y capacidad maxima es 1
        )
        :effect (and
            (at ?p ?l) (not (loaded ?v ?p)) (not (fullloaded ?v))
        )
    )
